...
function injectForm(formId, obj,mp) {
    ...
    for (let objKey in obj) {
        ...
        var realName = mp[objKey] || objKey;
        var $obj = getObjOfForm(formId, realName).not("[fixed]");
        if ($obj.size() > 0) {
            var tagName  = $obj[0].tagName;
            if (tagName === 'IMG') {
                $obj.attr("src", obj[objKey]);
            } else {
                // console.log($obj);
...
function convertJObjToArr($arr,dtProp) {
    var ret = [];
    $arr.each(function (idx, ele) {
        ret.push($(ele).data(dtProp));
    });
    return ret;
}
function convertObjArr(arr,cb) {
    var ret = [];
    for(var i=0;i<arr.length;i++){
        var one = arr[i];
        var converted = cb(one);
...
function showOrHideFormInput(formId,v) {
    $("#"+formId).find("[show-mode]").each(function (idx, ele) {
        var $ele = $(ele);
        if ($ele.attr('show-mode') != v) {
            $ele.hide();
        } else {
            $ele.show();
        }
